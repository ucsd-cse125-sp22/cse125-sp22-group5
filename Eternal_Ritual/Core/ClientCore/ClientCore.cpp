...
void ClientCore::initEngine() {
    ...
    this->engine = new Engine("KGLEngine", 0.4f, 0, NULL);
    this->engine->workingDirectory = "/Users/kang/Desktop/Week6/Client/KGLEngine";
    this->engine->lockCursor();
}
...
void ClientCore::loadLight() {
    ...
    this->pointLight = new LightNode(vec3(5.0f));
    this->pointLight->setPointLight(2.0f, 20.0f);
    this->pointLight->highlightIntensity = 0.0f;
    
    this->ambientLight = new LightNode(vec3(0.5f));
    this->ambientLight->setAmbientLight();
    this->engine->addNode(ambientLight);
    
    this->directionalLight = new LightNode(vec3(2.5f, 2.0f, 2.0f));
    this->directionalLight->setDirectionalLight();
    this->directionalLight->eulerAngles = vec3(0.0f, 135.0f, -45.0f);
    this->engine->addNode(directionalLight);
    
//    spotLight = new LightNode(vec3(20.0f, 0.5f, 0.5f));
...
void ClientCore::loadScene() {
    ...
    this->engine->addNode(sceneNode);
    
    this->reflection = new Texture("/Resources/Game/PBR/RF.png", 0.0f, false);
    
    this->mapShader = new PBRShader(0.5f, 0.5f);
    this->mapShader->setReflectionMap(reflection);
...
void ClientCore::loadCharacter() {
    ...
    this->mixamoD = new Texture("/Resources/Game/Character/Textures/Mixamo D.png", 2.0f, true);
    this->mixamoN = new Texture("/Resources/Game/Character/Textures/Mixamo N.png", 2.0f, true);
    this->mixamoM = new Texture("/Resources/Game/Character/Textures/Mixamo M.png", 2.0f, true);
    
    this->mixamoMaterial = new PBRShader(0.0f, 0.0f);
    this->mixamoMaterial->setDiffuseMap(mixamoD);
    this->mixamoMaterial->setNormalMap(mixamoN);
    this->mixamoMaterial->setMetallicMap(mixamoM);
    this->mixamoMaterial->metallicIntensity = 2.0f;
    this->mixamoMaterial->setRoughnessMap(mixamoM);
    this->mixamoMaterial->invertRoughness = true;
    this->mixamoMaterial->setReflectionMap(reflection);
    
    this->character = new CharNode(vec3(0.0f, -1.0f, 0.0f));
    this->character->name = "main character";
    
    Node* controlNode = new Node();
    controlNode->position = vec3(0.0f, 1.0f, 0.0f);
    this->character->setControl(controlNode);
    
    CameraNode* cameraNode = new CameraNode(60.0f, 0.1f, 1000.0f);
    cameraNode->position = vec3(-2.0f, 0.0f, 0.0f);
    this->character->setCamera(cameraNode);
    
    cameraNode->addChildNode(pointLight);

    //engine->addNode(cameraNode);
    this->engine->mainCameraNode = cameraNode;
    ...
    this->engine->addNode(character);
    this->character->setModel(charModel);
    
    this->character->addAnimator("idle", "/Resources/Game/Character/Animations/Idle.dae");
    this->character->stopAndPlay("idle", 0.0f, 0.0f);
    
    this->character->addAnimator("running", "/Resources/Game/Character/Animations/Running.dae");
    
    this->character->addAnimator("back run", "/Resources/Game/Character/Animations/Back Run.fbx");
    
    this->character->addAnimator("left strafe", "/Resources/Game/Character/Animations/Left Strafe.fbx");
    
    this->character->addAnimator("right strafe", "/Resources/Game/Character/Animations/Right Strafe.fbx");
    
    this->character->addAnimator("roll", "/Resources/Game/Character/Animations/Roll.dae");
    
    this->character->addAnimator("cast magic 1", "/Resources/Game/Character/Animations/Cast Magic 1.fbx");
    
    UINode* baseNode = new UINode();
    baseNode->renderingOrder = 1000.0f;
    this->engine->addNode(baseNode);
    this->character->setUINode(baseNode);
    this->character->setName("Player1");
}
...
void ClientCore::loadWeapon() {
    ...
    this->weaponNode = new Node();
    this->weaponNode->scale = vec3(1);
    
    characterRightHand->addChildNode(weaponNode);
}


void ClientCore::initPbPacket() {
    std::cout << std::endl;
    std::cout << "|-- Stage 7 - Initial ProtoBuf Packet --|" << std::endl;
    
    this->statePb = new StatePb();
    this->eventPb = new EventPb();
}


void ClientCore::connectServer() {
    std::cout << std::endl;
    std::cout << "|-- Stage 8 - Connect to Server --|" << std::endl;
...
void ClientCore::handleEvent() {
    ...
    if (engine->isRunning()) {
        ...
        if(engine->input->wasKeyPressed(KEY_0)) {
            engine->unlockCursor();
        }
        
        if(engine->input->wasKeyPressed(KEY_MINUS)) {
            engine->lockCursor();
        }
        ...
        if (!this->startGame) {
            this->character->moveCamera(this->engine->input->getMouseTranslation() * 0.1f);
            this->eventPb->setControlNodeEulerAngles(this->character->controlNode->eulerAngles);
            
            
            if(engine->input->isPressingKey(KEY_W)) {
                this->eventPb->addMovEvent(gameDataPb::MOVEFRONT);
            }else if(engine->input->isPressingKey(KEY_S)) {
                this->eventPb->addMovEvent(gameDataPb::MOVEBACK);
            }
            
            if(engine->input->isPressingKey(KEY_A)) {
                this->eventPb->addMovEvent(gameDataPb::MOVELEFT);
            }else if(engine->input->isPressingKey(KEY_D)) {
                this->eventPb->addMovEvent(gameDataPb::MOVERIGHT);
            }
            
            
            if(engine->input->isPressingKey(KEY_1)) {
                if (this->keyToMagic.count(KEY_1)) {
                    this->eventPb->addMagicEvent(gameDataPb::MagicPb(this->keyToMagic[KEY_1]));
                }
            }
            
            if(engine->input->isPressingKey(KEY_2)) {
                if (this->keyToMagic.count(KEY_2)) {
                    this->eventPb->addMagicEvent(gameDataPb::MagicPb(this->keyToMagic[KEY_2]));
                }
            }
            
            if(engine->input->isPressingKey(KEY_3)) {
                if (this->keyToMagic.count(KEY_3)) {
                    this->eventPb->addMagicEvent(gameDataPb::MagicPb(this->keyToMagic[KEY_3]));
                }
            }
            
            if(engine->input->isPressingKey(KEY_4)) {
                if (this->keyToMagic.count(KEY_4)) {
                    this->eventPb->addMagicEvent(gameDataPb::MagicPb(this->keyToMagic[KEY_4]));
                }
            }
            
            if(engine->input->isPressingKey(KEY_5)) {
                if (this->keyToMagic.count(KEY_5)) {
                    this->eventPb->addMagicEvent(gameDataPb::MagicPb(this->keyToMagic[KEY_5]));
                }
            }
            
            if(engine->input->isPressingKey(KEY_6)) {
                this->eventPb->addAddMagic(gameDataPb::MagicPb(STONEMAGIC));
            }
            
            if(engine->input->isPressingKey(KEY_7)) {
                this->eventPb->addRmMagic(gameDataPb::MagicPb(STONEMAGIC));
            }
            
            if(engine->input->isPressingKey(KEY_8)) {
                this->eventPb->addAddMagic(gameDataPb::MagicPb(FIREBALL));
            }
            
            if(engine->input->isPressingKey(KEY_9)) {
                this->eventPb->addRmMagic(gameDataPb::MagicPb(FIREBALL));
            }
        }
    }
}
...
void ClientCore::sendData() {
    ...
    char* pbArr = this->eventPb->sendData();
    int dataLen = this->eventPb->getDataLen();
    
    this->clientSocket->sendData(pbArr, dataLen);
    this->eventPb->clearData();
}
...
void ClientCore::receiveData() {
    ...
    this->clientSocket->receiveData();
}


void ClientCore::processData(char* pbArr, int dataLen) {
    this->statePb->clearData();
    this->statePb->readData(pbArr, dataLen);
}
...
void ClientCore::updateState() {
    ...
    if (startGame) {
        this->characterIP = this->statePb->getPlayerIP();
        this->character->position = this->statePb->getPosition(this->characterIP);
        
        this->enemyIPs = this->statePb->getEnemyIPs(this->characterIP);
        
        for (auto enemyIP : this->enemyIPs) {
            vec3 enemyPosition = this->statePb->getPosition(enemyIP);
            CharNode* enemy = character->copy(enemyPosition);
            enemy->name = "enemy";
            ...
            this->enemies[enemyIP] = enemy;
            
            
            Node* enemyRightHand = enemy->generateBoneNode("RightHand");
            
            Node* weaponNode = new Node();
            weaponNode->scale = vec3(1);
            
            enemyRightHand->addChildNode(weaponNode);
            this->enemyWeapons[enemyIP] = weaponNode;
        }
        
        this->startGame = false;
    }
    else {
        this->character->keyDirection = direction(this->statePb->getDirState(this->characterIP));
        this->character->characterTargetPosition = this->statePb->getPosition(this->characterIP);
        this->character->characterTargetEulerAngles = this->statePb->getEulerAngle(this->characterIP);

        this->character->updatePosition();
        this->character->updateTransform();
        
        if (this->statePb->hasRmMagics(this->characterIP)) {
            vector<gameDataPb::MagicPb> rmMagics = this->statePb->getRmMagics(this->characterIP);
            for (auto it : rmMagics) {
                Magic rmMagic = Magic(it);
                if (!this->character->magics.count(rmMagic)) {
                    continue;
                }
                
                this->keyToMagic.erase(this->magicToKey[rmMagic]);
                this->magicToKey.erase(rmMagic);

                BaseMagic* tempMagic = this->character->rmMagics(rmMagic);
                
                if (tempMagic != nullptr) {
                    this->allMagics.erase(tempMagic);
                    tempMagic->removeFromParentNode();
                }
            }
        }
        
        if (this->statePb->hasAddMagics(this->characterIP)) {
            vector<gameDataPb::MagicPb> addMagics = this->statePb->getAddMagics(this->characterIP);
            for (auto it : addMagics) {
                Magic addMagic = Magic(it);
                if (this->character->magics.count(addMagic)) {
                    continue;
                }
                
                if (!this->keyToMagic.count(KEY_1)) {
                    this->keyToMagic[KEY_1] = addMagic;
                    this->magicToKey[addMagic] = KEY_1;
                } else if (!this->keyToMagic.count(KEY_2)) {
                    this->keyToMagic[KEY_2] = addMagic;
                    this->magicToKey[addMagic] = KEY_2;
                } else if (!this->keyToMagic.count(KEY_3)) {
                    this->keyToMagic[KEY_3] = addMagic;
                    this->magicToKey[addMagic] = KEY_3;
                } else if (!this->keyToMagic.count(KEY_4)) {
                    this->keyToMagic[KEY_4] = addMagic;
                    this->magicToKey[addMagic] = KEY_4;
                } else if (!this->keyToMagic.count(KEY_5)) {
                    this->keyToMagic[KEY_5] = addMagic;
                    this->magicToKey[addMagic] = KEY_5;
                }
                
                BaseMagic* newMagic;
                switch (addMagic) {
                    case STONEMAGIC:
                        newMagic = new StoneBlast();
                        this->engine->addNode(newMagic);
                        break;
                    case FIREBALL:
                        newMagic = new FireBall();
                        break;
                        

                }
                this->character->addMagics(newMagic, addMagic);
                this->allMagics.insert(newMagic);
                
            }
        }
        
        if (this->statePb->hasMagicEvents(this->characterIP)) {
            vector<gameDataPb::MagicPb> magicEvents = this->statePb->getMagicEvents(this->characterIP);
            for (auto it : magicEvents) {
                Magic magicEvent = Magic(it);
                
                BaseMagic* charMagic = this->character->magics[magicEvent];
                switch (magicEvent) {
                    case STONEMAGIC:
                        break;
                    case FIREBALL:
                        if (charMagic->start) {
                            weaponNode->addChildNode(charMagic);
                        }
                        break;
                }
                this->character->castMagic(magicEvent);
            }
        }
        
        
        for (auto& it : this->enemies) {
            ulong enemyIP = it.first;
            CharNode* enemy = it.second;
            
            enemy->keyDirection = direction(this->statePb->getDirState(enemyIP));
            enemy->characterTargetPosition = this->statePb->getPosition(enemyIP);
            enemy->characterTargetEulerAngles = this->statePb->getEulerAngle(enemyIP);

            enemy->updatePosition();
            enemy->updateTransform();
            
            
            if (this->statePb->hasRmMagics(enemyIP)) {
                vector<gameDataPb::MagicPb> rmMagics = this->statePb->getRmMagics(enemyIP);
                for (auto it : rmMagics) {
                    Magic rmMagic = Magic(it);
                    if (!enemy->magics.count(rmMagic)) {
                        continue;
                    }

                    BaseMagic* tempMagic = enemy->rmMagics(rmMagic);
            ...
            if (this->statePb->hasAddMagics(enemyIP)) {
                vector<gameDataPb::MagicPb> addMagics = this->statePb->getAddMagics(enemyIP);
                for (auto it : addMagics) {
                    Magic addMagic = Magic(it);
                    if (enemy->magics.count(addMagic)) {
                        continue;
                    }
                    ...
                    enemy->addMagics(newMagic, addMagic);
                    this->allMagics.insert(newMagic);
                    
                }
            }
            
            if (this->statePb->hasMagicEvents(enemyIP)) {
                vector<gameDataPb::MagicPb> magicEvents = this->statePb->getMagicEvents(enemyIP);
                for (auto it : magicEvents) {
                    Magic magicEvent = Magic(it);
                    
                    BaseMagic* charMagic = enemy->magics[magicEvent];
                    switch (magicEvent) {
                        case STONEMAGIC:
                            break;
                        case FIREBALL:
                            if (charMagic->start) {
                                enemyWeapons[enemyIP]->addChildNode(charMagic);
                            }
                            break;
                    }
                    enemy->castMagic(magicEvent);
                }
            }
        }
        
        
        for (auto magic : this->allMagics) {
            magic->updateMagic();
        }
    }
}
...
void ClientCore::closeConnect() {
    ...
    this->clientSocket->closeSockets();
}
