...
ServerCore::ServerCore() {
    this->tickEnd = new timeval();
    this->currTime = new timeval();
    this->serverCycle = 0;
}


ServerCore::~ServerCore() {
    for (auto& eventPb : eventPbs) {
...
void ServerCore::openServer() {
    ...
    this->serverSocket->startListen();
    
    std::cout << "Ready to connect all clients..." << std::endl;
    std::cout << std::endl;
    
    this->playerIPs = this->serverSocket->connectAllClients();
...
void ServerCore::initPbPacket() {
    for (auto playerIP : this->playerIPs) {
        this->eventPbs[playerIP] = new EventPb();
    }
    this->statePb = new StatePb();
}

void ServerCore::initEngine() {
    this->engine = new Engine("KGLEngine", 0.8f, 0, NULL);
    this->engine->workingDirectory = "/Users/kang/Desktop/Week6/Server/KGLEngine";
    this->engine->lockCursor();
}

void ServerCore::loadScene() {
...
void ServerCore::loadCharacters() {
    for (auto playerIP : this->playerIPs) {
        ...
        this->characters[playerIP] = character;
        
        Node* enemyRightHand = character->generateBoneNode("RightHand");
        
        Node* weaponNode = new Node();
        weaponNode->scale = vec3(1);
        
        enemyRightHand->addChildNode(weaponNode);
        this->characterWeapons[playerIP] = weaponNode;
    }
}
...
void ServerCore::setTickEnd() {
    ...
    std::cout << std::endl;
    std::cout << std::endl;
    std::cout << "Server Cycle " << ++serverCycle << ":" << std::endl;
    std::cout << "-----------------------------" << std::endl;
    
    gettimeofday(tickEnd, NULL);
    tickEnd->tv_usec += 33000;
    if (tickEnd->tv_usec >= 1000000) {
        tickEnd->tv_usec -= 1000000;
        tickEnd->tv_sec += 1;
    }
}
...
void ServerCore::receiveData() {
    ...
    this->serverSocket->receiveData();
}


void ServerCore::processData(ulong playerIP, char* pbArr, int dataLen) {
    if (eventPbs.count(playerIP)) {
        eventPbs[playerIP]->clearData();
        eventPbs[playerIP]->readData(pbArr, dataLen);
    }
}
...
void ServerCore::updateState() {
    ...
    for (auto playerIP : this->playerIPs) {
        CharNode* character = this->characters[playerIP];
        EventPb* eventPb = this->eventPbs[playerIP];
        
        if (startGame) {
            character->position = vec3(0.0f, -1.0f, 0.0f);
            this->statePb->setPosition(playerIP, character->position);
        }
        else {
            character->controlNode->eulerAngles = eventPb->getControlNodeEulerAngles();
            queue<gameDataPb::MovEvent> moveEvents = eventPb->getMovEvents();
            if (!moveEvents.empty()) {
                if(moveEvents.front() == gameDataPb::MOVEFRONT) {
                    moveEvents.pop();
                    character->moveFront();
                }else if(moveEvents.front() == gameDataPb::MOVEBACK) {
                    moveEvents.pop();
                    character->moveBack();
                }
            }
            if (!moveEvents.empty()) {
                if(moveEvents.front() == gameDataPb::MOVELEFT) {
                    moveEvents.pop();
                    character->moveLeft();
                }else if(moveEvents.front() == gameDataPb::MOVERIGHT) {
                    moveEvents.pop();
                    character->moveRight();
                }
            }
            ...
            this->statePb->setDirState(playerIP, gameDataPb::DirState(character->keyDirection));
            this->statePb->setPosition(playerIP, character->characterTargetPosition);
            this->statePb->setEulerAngle(playerIP, character->characterTargetEulerAngles);
            
            character->updateTransform();
            
            
            if (eventPb->hasRmMagics()) {
                vector<gameDataPb::MagicPb> rmMagics = eventPb->getRmMagics();
                for (auto it : rmMagics) {
                    Magic rmMagic = Magic(it);
                    if (!character->magics.count(rmMagic)) {
                        continue;
                    }

                    BaseMagic* tempMagic = character->rmMagics(rmMagic);
                    ...
                    this->statePb->addRmMagic(playerIP, it);
                }
            }
            
            if (eventPb->hasAddMagics()) {
                vector<gameDataPb::MagicPb> addMagics = eventPb->getAddMagics();
                for (auto it : addMagics) {
                    Magic addMagic = Magic(it);
                    if (character->magics.count(addMagic)) {
                        continue;
                    }
                    ...
                    character->addMagics(newMagic, addMagic);
                    this->allMagics.insert(newMagic);
                    
                    this->statePb->addAddMagic(playerIP, it);
                }
            }
            
            if (eventPb->hasMagicEvents()) {
                vector<gameDataPb::MagicPb> magicEvents = eventPb->getMagicEvents();
                for (auto it : magicEvents) {
                    Magic magicEvent = Magic(it);
                    BaseMagic* charMagic = character->magics[magicEvent];
                    switch (magicEvent) {
                        case STONEMAGIC:
                            break;
                        case FIREBALL:
                            if (charMagic->start) {
                                characterWeapons[playerIP]->addChildNode(charMagic);
                            }
                            break;
                    }
                    ...
                    this->statePb->addMagicEvent(playerIP, it);
                }
            }
        }
    }
...
void ServerCore::sendData() {
    ...
    if (startGame) {
        for (auto playerIP : this->playerIPs) {
            this->statePb->setPlayerIP(playerIP);
            
            char* pbArr = this->statePb->sendData();
            int dataLen = this->statePb->getDataLen();
            
            this->serverSocket->sendData(playerIP, pbArr, dataLen);
        }
...
void ServerCore::waitTickEnd() {
    ...
    } while (timeval_usecdiff(currTime, this->tickEnd) > 0);
}
