...
PBRShader* PBRShader::copy() {
    ...
    for(unsigned int i = 0; i < this->uniformTextureNames.size(); i += 1) {
        ...
        }else if(this->uniformTextureNames[i] == "ORMMap") {
            this->setBool("useORMMap", true);
        }
    }
...
void PBRShader::setORMMap(Texture* texture) {
    this->setBool("useORMMap", true);
    this->setTexture("ORMMap", texture);
}
