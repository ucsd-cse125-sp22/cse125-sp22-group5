...
void main() {
    ...
    float noiseFactor = texture(noiseMap, UV).r;
    if(noiseFactor < deathFactor * 1.05f) {
        if(noiseFactor < deathFactor * 1.05f - 0.05f) {
            discard;
        }else{
            float factor = (deathFactor * 1.05f - noiseFactor) / 0.05f;
            emissionColor.r += factor;
        }
    }
    
    float maskNoiseFactor1 = texture(noiseMap, UV * vec2(0.5f, 2.0f) + vec2(time * 0.1f, 0.0f)).r;
    float maskNoiseFactor2 = texture(noiseMap, UV * vec2(1.0f, 5.0f) + vec2(-time * 0.2f, 0.0f)).r;
    float maskNoiseFactor = (maskNoiseFactor1 + maskNoiseFactor2) * 0.5f;
    float maskFactor = texture(maskMap, UV).r * fragment.positionalMaskFactor - 0.1f;
    if(maskFactor < maskNoiseFactor) {
        if(maskFactor < maskNoiseFactor - 0.1f) {
            discard;
        }else{
            float factor = (maskNoiseFactor - maskFactor) / 0.1f;
            emissionColor.r += factor;
        }
    }
    emissionColor.r += maskNoiseFactor * 0.1f;
    
    if(renderingMode == 1) {
        return;
    }
    if(!gl_FrontFacing) {
        color = vec4(maskNoiseFactor, 0.0f, 0.0f, 1.0f);
        return;
    }
    ...
    color = vec4(kD * diffuseColor.rgb + lightingColor * shadowAmount, 1.0f);
    color.rgb = color.rgb / (color.rgb + vec3(1.0f));
    color.rgb += texture(emissionMap, UV).rgb * (1.0f - deathFactor);
