...
#include "input.hpp"
Input::KeyEvent::KeyEvent(int key, int state, double time) {
    this->key = key;
    this->state = state;
...
void Input::engineKeyEvent(int key, int state, double time, string character) {
...
double Input::getKeyDuration(int key, double currentTime) {
    int i = 0;
    while(i < this->keyEvents.size()) {
        if(this->keyEvents[i].key == key) {
